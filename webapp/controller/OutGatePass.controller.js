sap.ui.define([
	...
	return BaseController.extend("zaudgpms.audhatham.com.controller.OutGatePass", {
		...
		onGenerateDC: async function () {
			...
			doc.setFont("helvetica", "normal"); doc.text("Madhapur, Hyderabad", c3 + valOff3, y3);
			y3 += lh + 2;
			doc.setFont("helvetica", "bold"); doc.text("Despatch To:", c3, y3);
			doc.setFont("helvetica", "normal"); doc.text(oOut.City || "Hyderabad", c3 + valOff3, y3);
			y3 += lh + 2;
			...
			doc.setFont("helvetica", "normal");
			doc.setFontSize(7);
			doc.text("For Audhataam Pharmaceuticals Private Limited", pageWidth / 2, sigY + 10, { align: "center" });
		...
		onGenerateEWay: async function () {
			...
			doc.text("GSTIN : " + (oOut.CocodeGST || "36AABCA8375L1Z1"), margin + 2, y + 5);
			doc.text("AUDHATAAM PHARMACEUTICALS PRIVATE LIMITED", margin + 2, y + 10);
			...
			doc.autoTable({
				...
				body: [['Road', oOut.VehicleNo || "", 'Hyderabad', sDate, 'System']],
				theme: 'grid',
				headStyles: { fillColor: [255, 255, 255], textColor: [0, 0, 0], fontStyle: 'bold', lineWidth: 0.1 },
		...
		onPrintGatePass: async function () {
			...
			doc.setFont("helvetica", "italic");
			doc.setFontSize(8);
			doc.text("to take out the following material from Audhataam premises.", lColX + pad, gridY + gridH - 3.5);
